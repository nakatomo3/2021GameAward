    ...
    void Update() {
        if (Player.instance.isAlive == true) {
            ...
            for (int i = 0; i < ghosts.Count; ++i) {
                ...
                animators[i].SetBool("isIdel", false);
                animators[i].SetBool("isMove", true);
            }


            if (moveStepRate / Player.instance.moveIntervalMax >= 1) {
                moveStepRate = Player.instance.moveIntervalMax;

                for (int i = 0; i < ghosts.Count; ++i) {
                    oldPos[i] = newPos[i];
                    animators[i].SetBool("isIdel", true);
                    animators[i].SetBool("isMove", false);
                }
            }
